...
export const TIMEZONE_REGIONS: TimezoneRegion[] = [
  {
    ...
    label: "Europe",
    cities: [
      { code: "moscow", label: "Moscow", iana: "Europe/Moscow" },
      { code: "kyiv", label: "Kyiv", iana: "Europe/Kyiv" },
      { code: "london", label: "London", iana: "Europe/London" },
      { code: "berlin", label: "Berlin", iana: "Europe/Berlin" },
      { code: "paris", label: "Paris", iana: "Europe/Paris" },
      { code: "istanbul", label: "Istanbul", iana: "Europe/Istanbul" },
    ],
  },
  {
    code: "americas",
    label: "Americas",
    cities: [
      { code: "ny", label: "New York", iana: "America/New_York" },
      { code: "chicago", label: "Chicago", iana: "America/Chicago" },
      { code: "la", label: "Los Angeles", iana: "America/Los_Angeles" },
      { code: "toronto", label: "Toronto", iana: "America/Toronto" },
      { code: "saopaulo", label: "São Paulo", iana: "America/Sao_Paulo" },
    ],
  },
  {
    code: "asia",
    label: "Asia",
    cities: [
      { code: "dubai", label: "Dubai", iana: "Asia/Dubai" },
      { code: "tashkent", label: "Tashkent", iana: "Asia/Tashkent" },
      { code: "almaty", label: "Almaty", iana: "Asia/Almaty" },
      { code: "tokyo", label: "Tokyo", iana: "Asia/Tokyo" },
      { code: "singapore", label: "Singapore", iana: "Asia/Singapore" },
    ],
  },
  {
    code: "pacific",
    ...
    cities: [
      ...
      { code: "sydney", label: "Sydney", iana: "Australia/Sydney" },
      { code: "auckland", label: "Auckland", iana: "Pacific/Auckland" },
    ],
  },
];
...
function backButton(prefix = SETTINGS_PREFIX): InlineKeyboard {
  return new InlineKeyboard().text("← Back", `${prefix}back`);
}

function timezoneBackButton(prefix = SETTINGS_PREFIX): InlineKeyboard {
  return new InlineKeyboard().text("← Regions", `${prefix}tz_back`);
}

export function buildTimezoneRegionText(current: string): string {
  return [`🌍 Timezone`, "", `Current: ${current}`, "", "Pick a region."].join("\n");
}
...
export function buildTimezoneCityText(regionCode: string, current: string): string {
  ...
  return [`🌍 ${name}`, "", `Current: ${current}`, "", "Pick a city."].join("\n");
}
...
export function buildDmTimezoneKeyboard(current: string | null): InlineKeyboard {
  ...
  return kb.text("Clear override", `${DM_PREFIX}clear`).row();
}

export function buildMenuKeyboard(showSave: boolean): InlineKeyboard {
  const kb = new InlineKeyboard()
  ...
  if (showSave) {
    kb.row()
      .text("✅ Save & enable", `${SETTINGS_PREFIX}save`)
      .text("✖ Cancel", `${SETTINGS_PREFIX}cancel`);
  } else {
    kb.row().text("✖ Close", `${SETTINGS_PREFIX}cancel`);
  }
